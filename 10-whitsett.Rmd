...
## Monthly call volume {#whitsettvolume}

How many calls do we receive each month?

```{r}
call_data %>%
  filter(jurisdiction == "Whitsett") %>%
...
## Nature Breakdown {#whitsettnatures}
...
## Location Overview {#whitsettlocation}
...
call_dat_sf %>%
  ...
  filter(common_calltime >= (floor_date(Sys.Date() - 365, unit = "1 month"))) %>%
  ggplot() +
  geom_sf(data = filter(guil_places, NAME == "Whitsett"), color = "grey", fill = NA) +
  geom_text(data = filter(places_labels, NAME == "Whitsett"), aes(long, lat, label = NAME), color = "black", size = 3) +
