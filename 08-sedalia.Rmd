...
## Monthly call volume {#sedaliavolume}

How many calls do we receive each month?

```{r}
call_data %>%
  filter(jurisdiction == "Sedalia") %>%
...
## Nature Breakdown {#sedalianatures}
...
## Location Overview {#sedalialocation}
...
call_dat_sf %>%
  ...
  filter(common_calltime >= (floor_date(Sys.Date() - 365, unit = "1 month"))) %>%
  ggplot() +
  geom_sf(data = filter(guil_places, NAME == "Sedalia"), color = "grey", fill = NA) +
  geom_text(data = filter(places_labels, NAME == "Sedalia"), aes(long, lat, label = NAME), color = "black", size = 3) +
