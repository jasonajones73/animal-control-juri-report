...
## Monthly call volume {#greensborovolume}

How many calls do we receive each month?

```{r}
call_data %>%
  filter(jurisdiction == "Greensboro") %>%
  ...
  geom_text(aes(label = scales::comma(total)), color = "white", nudge_y = -30) +
  scale_y_continuous(labels = scales::comma_format())
```

## Nature Breakdown {#greensboronatures}
...
call_data %>%
  ...
  geom_col(fill = "#1d4c94") +
  geom_text(aes(label = scales::comma(total)), color = "white", nudge_y = -80) +
  scale_y_continuous(labels = scales::comma_format()) +
  coord_flip() +
  theme(axis.text.y = element_text(size = 10))
```

## Location Overview {#greensborolocation}
...
call_dat_sf %>%
  ...
  filter(common_calltime >= (floor_date(Sys.Date() - 365, unit = "1 month"))) %>%
  ggplot() +
  geom_sf(data = filter(guil_places, NAME == "Greensboro"), color = "grey", fill = NA) +
  geom_text(data = filter(places_labels, NAME == "Greensboro"), aes(long, lat, label = NAME), color = "black", size = 3) +
  geom_hex(data = for_map, aes(X, Y), alpha = 0.4, show.legend = FALSE) +
