...
## Monthly call volume {#oakridgevolume}

How many calls do we receive each month?

```{r}
call_data %>%
  filter(jurisdiction == "Oak Ridge") %>%
...
## Nature Breakdown {#oakridgenatures}
...
## Location Overview {#oakridgelocation}
...
call_dat_sf %>%
  ...
  filter(common_calltime >= (floor_date(Sys.Date() - 365, unit = "1 month"))) %>%
  ggplot() +
  geom_sf(data = filter(guil_places, NAME == "Oak Ridge"), color = "grey", fill = NA) +
  geom_text(data = filter(places_labels, NAME == "Oak Ridge"), aes(long, lat, label = NAME), color = "black", size = 3) +
