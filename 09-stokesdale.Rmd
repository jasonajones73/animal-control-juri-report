...
## Monthly call volume {#stokesdalevolume}

How many calls do we receive each month?

```{r}
call_data %>%
  filter(jurisdiction == "Stokesdale") %>%
  ...
  geom_text(aes(label = scales::comma(total)), color = "white", nudge_y = -.5) +
  scale_y_continuous(labels = scales::comma_format())
```

## Nature Breakdown {#stokesdalenatures}
...
## Location Overview {#stokesdalelocation}
...
call_dat_sf %>%
  ...
  filter(common_calltime >= (floor_date(Sys.Date() - 365, unit = "1 month"))) %>%
  ggplot() +
  geom_sf(data = filter(guil_places, NAME == "Stokesdale"), color = "grey", fill = NA) +
  geom_text(data = filter(places_labels, NAME == "Stokesdale"), aes(long, lat, label = NAME), color = "black", size = 3) +
