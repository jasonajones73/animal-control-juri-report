...
# Location Breakdown {#locationbreakdown}
...
call_dat_sf %>%
  ...
  filter(common_calltime >= (floor_date(Sys.Date() - 365, unit = "1 month"))) %>%
  ggplot() +
  stat_density_2d(data = data.frame(st_coordinates(for_map)), aes(X, Y, fill = ..level..), alpha = 0.6, geom = "polygon") +
  geom_sf(alpha = 0.2, color = "#1d4c94") +
